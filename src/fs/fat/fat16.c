...
#define GARYOS_FAT16_SIGNATURE      0x29
#define GARYOS_FAT16_FAT_ENTRY_SIZE 0x022
#define GARYOS_FAT16_BAD_SECTOR     0xFF7
#define GARYOS_FAT16_UNUSED         0x00

typedef unsigned int FAT_ITEM_TYPE;
#define FAT_ITEM_TYPE_DIRECTORY     0
#define FAT_ITEM_TYPE_FILE          1
// Fat directory entry attr bitmask
...
#define FAT_FILE_DEVICE             0x49
#define FAT_FILE_RESERVED           0x80

// both of follow represent the ones at boot.asm
...
struct fat_directory_item{
    ...
    uint16_t low_16bits_bits_first_cluster;
    uint32_t filesize;
...
struct fat_private{
    ...
    // Use to stream the file allocation table
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector){
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != GARYOS_ALL_OK){
    ...
    while(1){
        ...
        if (diskstreamer_read(stream, &item, sizeof(item)) != GARYOS_ALL_OK){
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory){
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != GARYOS_ALL_OK){

        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != GARYOS_ALL_OK){
...
int fat16_resolve(struct disk* disk){
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != GARYOS_ALL_OK){
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != GARYOS_ALL_OK){
...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len){
    ...
    if (item->ext[0] != 0x00 && item->[0] != 0x20){
...
struct fat_directory_item* fat16_clone_directory_item(struct fat_directory){
...
static int fat16_get_fat_entry(struct disk* disk, int cluster){
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * GARYOS_FAT16_FAT_ENTRY_SIZE));
...
// Gets the correct cluster to use based on the starting cluster and offset
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset){
    ...
    for (int i = 0; i < clusters_ahead; i++){
        ...
        if (entry == 0xFF8 || entry == 0xFFF){
            ...
            // We are at the las entry in the file
            res = -EIO;
            goto out;
        }

        // Sector marked as bad
        if (entry == GARYOS_FAT16_BAD_SECTOR){
...
static int fat16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out){
    ...
    int res = 0;
    struct fat_private* private = disk->fs_private;
    int size_of_cluster_bytes = private->header.primary_header.sectors_per_cluster * disk disk->sector_size;
    ...
    if (total > 0){
        ...
        // We still more to read
        res = fat16_read_internal_from_stream(disk, stream, cluster, offset + total_to_read, out + total_to_read)
    }
...
static int fat16_read_internal(struct disk* disk, int starting_cluster, int offset, int total, void* out){
    ...
    struct fat_private* fs_prviate = disk->fs_private;
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name){
    ...
    char tmp_filename[GARYOS_MAX_PATH];
    for (int = 0; i < directory->total; i++){
...
void* fat16_open(struct disk* disk, struct path_part* path, FILE_MODE mode){
    ...
    descriptor = kzalloc(sizeof(fat_file_descriptor));
    if (!descriptor){
